...
def clear_all_messages():
    """Delete existing chat history when explicitly requested.

    Set CLEAR_MESSAGES_ON_STARTUP=true for one deployment only, then remove it.
    A new Neon database starts empty, so this is mainly useful for clearing the old DB.
    """
    if DATABASE_URL:
        with psycopg.connect(DATABASE_URL) as connection:
            ...
            connection.commit()
        print("All PostgreSQL chat history deleted.")
    else:
        connection = get_sqlite_connection()
        connection.execute("DELETE FROM messages")
        connection.commit()
        connection.close()
        print("All local SQLite chat history deleted.")


def init_database():
    if DATABASE_URL:
        print("Using PostgreSQL database (works with Neon free PostgreSQL).")
        with psycopg.connect(DATABASE_URL) as connection:
            with connection.cursor() as cursor:
                cursor.execute("""
...
# Optional one-time cleanup. Set CLEAR_MESSAGES_ON_STARTUP=true for one deploy
# if you want to delete every message from the currently connected database.
if os.environ.get("CLEAR_MESSAGES_ON_STARTUP", "").strip().lower() in {"1", "true", "yes"}:
    clear_all_messages()
